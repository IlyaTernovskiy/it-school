from back.models.models import (PVT, IprCalcRequest, NodalCalcRequest,
                                VlpCalcRequest)
from back.models.models import WellModelCalcRequest
...
def form_vlp_request(well_model_request: WellModelCalcRequest):
    ...
    return VlpCalcRequest(inclinometry=inclinometry, casing=casing, tubing=tubing, pvt=pvt, p_wh=p_wh, geo_grad=geo_grad, h_res=h_res).dict()
...
def form_nodal_request(vlp_result, ipr_result):
    return NodalCalcRequest(ipr=ipr_result, vlp=vlp_result).dict()
